...
// Set the heap to 128MB so we can easily hold multiple 1024x768x4 window
// buffers
...
void kfree(void *ptr) {
  ...
  if (!target)
    return; // Pointer not found in heap
...
void *kmemcpy32(void *dest, const void *src, uint32_t n) {
  __asm__ volatile("rep movsl" : "+D"(dest), "+S"(src), "+c"(n) : : "memory");
  return dest;
}
...
void *kmalloc(uint32_t size) {
  ...
  while (curr) {
    if (curr->is_free && curr->size >= size) {
      ...
      // The requested data + current header + NEW header + at least 16 bytes of
      // data.
      ...
      if (curr->size >= total_needed_to_split) {
        ...
        header_t *next_block =
            (header_t *)((uint32_t)curr + sizeof(header_t) + size);

        // The remainder is the current size minus what we took and the new
        // header
        ...
        // Point current to the new block, effectively inserting it into the
        // list
...
void *kmemset(void *dest, uint32_t val, uint32_t n) {
  __asm__ volatile("rep stosl" : "+D"(dest), "+c"(n) : "a"(val) : "memory");
  return dest;
}
...
void kheap_dump_map() {
  ...
  while (curr) {
    ...
    kprintf_unsync(
        "B%d: [%s] Addr: 0x%x | Data: 0x%x | Size: %d | Next: 0x%x\n", i++,
        curr->is_free ? "FREE" : "USED", (uint32_t)curr, data_start, curr->size,
        (uint32_t)curr->next);
