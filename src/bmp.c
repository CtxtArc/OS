...
void load_desktop_wallpaper(const char* filename) {
    ...
    struct bmp_info_header* info_hdr = (struct bmp_info_header*)(raw_data + sizeof(struct bmp_file_header));
    ...
    for (int y = 0; y < info_hdr->height && y < (int)sh; y++) {
        ...
        if (draw_y >= (int)sh) draw_y = sh - 1; // Crop if image is taller than screen
