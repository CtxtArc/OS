...
void map_page(uint32_t virtual_addr, uint32_t physical_addr) {
  ...
  if (dir_index < 32) {
    // THE FIX: Use 7 (User + R/W + Present) instead of 3
    kernel_page_tables[dir_index][table_index] = (physical_addr & ~0xFFF) | 7;
    flush_tlb();
  }
}
...
void paging_init(struct multiboot_info *mbi) {
  ...
  for (int t = 0; t < 32; t++) { // 32 tables * 4MB = 128MB mapped!
    for (int i = 0; i < 1024; i++) {
      ...
      // THE FIX: Use 7 (1 + 2 + 4) to allow Ring 3 execution and stack access
      kernel_page_tables[t][i] = phys | 7;
    }
    // THE FIX: Also apply 7 to the directory entry!
  ...
  for (int t = 0; t < 2; t++) {
    for (int i = 0; i < 1024; i++) {
      ...
      // VRAM can also be user-accessible if we want GUI apps to draw directly,
      // but since they go through the Compositor Syscalls, 3 or 7 is fine here.
      vesa_page_tables[t][i] = (actual_phys & ~0xFFF) | 7;
    }

    if ((dir_idx_start + t) < 1024) {
      page_directory[dir_idx_start + t] = ((uint32_t)vesa_page_tables[t]) | 7;
    }
  }
